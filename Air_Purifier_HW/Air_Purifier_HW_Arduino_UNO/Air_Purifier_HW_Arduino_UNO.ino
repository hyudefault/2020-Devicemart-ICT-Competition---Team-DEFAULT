...
void setup() {
  ...
  pinMode(ledPin1,OUTPUT);            // 빨간색 LED
  pinMode(ledPin2,OUTPUT);            // 노란색 LED
  pinMode(ledPin3,OUTPUT);            // 초록색 LED
...
void mode_setting_fn(int ModeState)     // Mode에 따른 동작 함수
{
  if(ModeState==0) {                    // Mode 0 이면,
    ...
    lcd.clear();
    lcd.setCursor(0,0);                 // 커서를 첫 번째 줄에 가져다 놓기
  ...
  if(ModeState==1) {                    // Mode 1 이면,
    ...
    digitalWrite(ledPin3,HIGH);         // 초록색 LED 켜기
    digitalWrite(ledPin2,LOW);
    digitalWrite(ledPin1,LOW);
  ...
  if(ModeState==4) {                    // Mode 4 이면,
    ...
    digitalWrite(ledPin3,HIGH);         // 모든 LED 점등
    digitalWrite(INA,HIGH);             
    digitalWrite(INB,LOW);              
